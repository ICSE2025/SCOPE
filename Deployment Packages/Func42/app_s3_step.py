...
def detect_object(index, index2, image, src_video, milli_1, milli_2, detect_prob):
    ...
    f = open(filename, "wb")
    key = image
    s3_client.download_fileobj("bucketwendycyn1", key , f, Config=config)
    f.close()
    ...
    s3_client.upload_file("/tmp/" + zipFileName, "bucketwendycyn1", "Detected_Objects/" + zipFileName, Config=config)
